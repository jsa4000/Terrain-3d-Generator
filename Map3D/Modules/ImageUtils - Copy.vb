...
Public Class ColorRange
    ...
    Public Function CheckValue(Value As Double) As Boolean
        ...
        If (Math.Round(Value, 6) >= Math.Round(FromValue, 6) AndAlso Math.Round(Value, 6) <= Math.Round(ToValue, 6)) Then
            Return True
        End If
        Return False
    End Function

    Public Function GetValue(value As Double) As Integer
        Dim result As Integer = ToColor
        'Check if the value is in the same range
        ...
        '        Dim colorfrom As String = Hex(FromColor).PadLeft(8, "0"c) 'Pad with 0's for alpha values = 0
        '        Dim colorTo As String = Hex(ToColor).PadLeft(8, "0"c) 'Pad with 0's for alpha values = 0

        '        ' Get the value (brightness) from and to
        '        Dim startValue As Integer = Int(0.2126 * (Convert.ToInt32(colorfrom.Substring(2, 2), 16)) + 0.7152 * (Convert.ToInt32(colorfrom.Substring(4, 2), 16)) + 0.0722 * (Convert.ToInt32(colorfrom.Substring(6, 2), 16)))
        '        Dim endValue As Integer = Int(0.2126 * (Convert.ToInt32(colorTo.Substring(2, 2), 16)) + 0.7152 * (Convert.ToInt32(colorTo.Substring(4, 2), 16)) + 0.0722 * (Convert.ToInt32(colorTo.Substring(6, 2), 16)))
        '        ' Get the rgba color for this value
        '        Dim valuePercentage As Double = ((endValue - startValue) * percentage) + startValue
        '        result = ImageUtils.Grayscale(valuePercentage).ToArgb
        '    End If
    ...
    Public Function GetColor(Value As Double) As Integer
        ...
        If (CheckValue(Value)) Then
            ...
            If (IsGrayScale) Then
                Dim total As Double = ToValue - FromValue
                If (total = 0) Then total = 1
                Dim percentage As Double = (Value - FromValue) / total
                result = ImageUtils.Grayscale(Convert.ToInt32(255 * percentage)).ToArgb
            End If
        End If
        Return result
    End Function
...
Public Class ImageUtils
    ...
    Public Shared PARALLEL_PROCESSES As Integer = 1
    ...
    Public Shared Function GetColorFromPalette(Value As Double, palette As List(Of ColorRange)) As Integer
        ...
        If (Not found) Then
            Dim temp As Double = Math.Round(palette(8).FromValue, 6)
